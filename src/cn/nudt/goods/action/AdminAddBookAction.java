...
@Controller
public class AdminAddBookAction extends ActionSupport {
	@Resource
	...
	public String addBook() throws ServletException, IOException {
		...
		try {
			...
			flag=false;
			return error("上传的文件超出了80KB", request, response);
		}
		...
		Admin sessionAdmin=(Admin) request.getSession().getAttribute("sessionAdmin");
		Category category = CommonUtils.toBean(map, Category.class);// 把Map中cid封装到Category中
		book.setCategory(category);
		book.setAdmin(sessionAdmin);
		...
		// 校验文件名称的扩展名
		if (!filename.toLowerCase().endsWith(".jpg")&&flag==true) {
			flag=false;
			return error("上传的图片扩展名必须是JPG", request, response);
		}
		// 校验图片的尺寸
		...
		// 3. 获取宽高来进行校验
		if ((image.getWidth(null) > 350 || image.getHeight(null) > 350)&&flag==true) {
		...
		if ((image.getWidth(null) > 350 || image.getHeight(null) > 350)&&flag==true) {
			destFile.delete();// 删除图片
			flag=false;
			return 	error("您上传的图片尺寸超出了350*350！", request, response);
		}else if(flag==true) {
	...
	public void validate() {
		...
		if (sessionAdmin==null|(adminService.login(sessionAdmin)==null)) {
			...
			this.addFieldError("msg", "管理员不能添加图书！");
			req.setAttribute("msg", "管理员不能添加图书！");
		}
	}
}
