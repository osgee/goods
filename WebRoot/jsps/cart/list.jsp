...
<script type="text/javascript">
	...
	function sendUpdateQuantity(id, quantity) {
		$.ajax({
			async : false,
			cache : false,
			url : "/goods/CartItem_updateQuantity.action",
			data : {
	...
	function batchDelete() {
		...
		location = "/goods/CartItem_batchDelete.action?cartItemIds="
				+ cartItemIdArray;
	}
...
<body>
	<c:choose>
		...
		<c:otherwise>
			<a>
			<label class="error" id="msg" style="margin-left: 250px;color:#cc3300;font-weight: 900;">${msg}
			</label>
			</a>
			<div class="content">
			<table width="95%" align="center" cellpadding="0" cellspacing="0">
				<tr align="center" bgcolor="#efeae5">
					<td align="left" width="50px"><a href="<c:url value='/CartItem_myCart.action'/>"><input type="checkbox"
						id="selectAll" checked="checked" /><label for="selectAll">全选</label></a>
					</td>
					<td width="800" colspan="2">商品名称</td>
					<td width="300">单价</td>
					<td width="300">数量</td>
					<td width="300">小计</td>
					<td width="300">操作</td>
				...
				<c:forEach items="${cartItemList }" var="cartItem">
					<tr align="center">
						...
						<td align="left" width="70px"><a class="linkImage"
							href="<c:url value='/goods/Book_load.action?bid=${cartItem.book.bid }'/>"><img
								border="0" width="54" align="top"
								src="<c:url value='/${cartItem.book.image_b }'/>" /></a></td>
						<td align="left" width="400px"><a
							href="<c:url value='/goods/Book_load.action?bid=${cartItem.book.bid}'/>"><span
								style="margin-left: 0px;margin-right: 0px">${cartItem.book.bname }</span></a>
						</td>
						<td><span>&yen;<span class="currPrice">${cartItem.book.currPrice }</span></span></td>
						<td><a class="jian" id="${cartItem.cartItemId }Jian"
							href="<c:url value='/CartItem_myCart.action'/>"></a><input
							class="quantity" readonly="readonly"
							id="${cartItem.cartItemId }Quantity" type="text"
							value="${cartItem.quantity }" /><a class="jia"
							id="${cartItem.cartItemId }Jia"
							href="<c:url value='/CartItem_myCart.action'/>"></a></td>
						<td width="100px"><span class="price_n">&yen;<span
								class="subTotal" id="${cartItem.cartItemId }Subtotal">${cartItem.subtotal }</span></span>
						</td>
						<td><a
							href="<c:url value='/CartItem_batchDelete.action?cartItemIds=${cartItem.cartItemId }'/>">删除</a>
						</td>
						<td><a>${cartItem.book.admin.adminname}</a></td>
					</tr>
				</c:forEach>
				<tr>
			...
			<form id="jieSuanForm"
				action="<c:url value='/CartItemloadCartItems.action'/>"
				method="post">
